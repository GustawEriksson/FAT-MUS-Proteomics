...
DEP_Preprocess <- function(P.matrix, exp.design, output.dir, name, vsn = FALSE, 
  ...
  #  #P.matrix[,17:46] <- as.numeric(unlist(P.matrix[,17:46]))
  ...
  # Performe variance stabilizing transformation (vsn) on data
  #Fat_vsn = normalize_vsn(Fat_filt)
  #Muscle_vsn = normalize_vsn(Muscle_filt)
  ...
  # Plotting an heatmap of proteins with missing values
  #plot_missval(P.filt)
  #ggsave(paste0(output.dir,name,"_Heatmap_Missing-Values.pdf"))
  ...
  P.log.ID = merge(P.unique[,47:48],P.log.ID, by = "name")
  rownames(P.log.ID) = P.log.ID$ID
